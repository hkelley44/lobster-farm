...
function build_reviewer_prompt(pr: WebhookPR, repo_path: string, issue_context: string): string {
  const lines = [
    ...
    "",
    "IMPORTANT: Do NOT check CI status or mention CI in your review.",
    "Your job is code quality only. Branch protection and the daemon handle CI gating.",
    "Never request changes because CI is pending or failing — that creates infinite loops.",
    "",
    "After posting your review:",
    "- If you approved, attempt to merge the PR:",
    `  gh pr merge ${String(pr.number)} --squash --delete-branch`,
    "  If the merge fails because checks are pending, that's fine — the daemon will merge it later.",
