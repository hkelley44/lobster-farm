...
export const entity_command = new Command("entity").description("Manage LobsterFarm entities");
  ...
  .action(
    async (options: {
      ...
      if (ni) {
        ...
        if (p.isCancel(github_org_result)) {
          p.cancel("Cancelled");
          process.exit(0);
        }
        ...
        if (p.isCancel(setup_discord_result)) {
          p.cancel("Cancelled");
          process.exit(0);
        }
      ...
      p.note(
        [
          ...
          channels.length > 0
            ? `Discord:     ${String(channels.length)} channels configured`
      ...
      p.outro(
        `Entity "${entity_id}" is ready. The daemon will pick it up on next restart (or reload).`,
      );
    },
  );
