...
export async function load_yaml<T>(file_path: string, schema: z.ZodSchema<T>): Promise<T> {
  const content = await readFile(file_path, "utf-8");
  return parse_yaml(content, schema);
}

/** Parse a YAML string and validate against a Zod schema. */
export function parse_yaml<T>(content: string, schema: z.ZodSchema<T>): T {
...
export async function write_yaml(file_path: string, data: unknown): Promise<void> {
